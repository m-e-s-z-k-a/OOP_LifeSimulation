...
public class SimulationEngine implements IEngine, Runnable {
    ...
    public SimulationEngine(AbstractMap map, int start_animals_number, boolean isMagic, GridPane gridPane, DataChart dataChart, GenotypeText text, FileData fileData) {
        ...
        do {
            ...
            for (int j = 0; j < 32; j++) {
                genotype[j] = new Random().nextInt(8);  // nowy obiekt co obrót pętli... wewnętrznej
            }
    ...
    public void mapUpdate() {   // na pewno public?
    ...
    public void run() {
        do {
            synchronized (lock) {
                ...
                if (pause) {
                    try {
                        synchronized (lock) {   // już mamy taki blok na zewnątrz
                            lock.wait();
                        }
            ...
            if (this.map.animals_number == 5 && this.isMagicGameplayOn && timesMagicUsed < 3) {
                ...
                timesMagicUsed += 1;
                Platform.runLater(() -> {
                    Text popup_text = new Text(this.map.getMapType() + " Magic! " + (3 - timesMagicUsed) + " time(s) left");    // to powinno być w GUI
